...
sqlite3 /etc/x-ui/x-ui.db "INSERT INTO settings VALUES ($(sqlite3 /etc/x-ui/x-ui.db "SELECT IFNULL(MAX(id), 0) FROM settings"), 'webKeyFile', '/etc/ssl/private/private.key'); INSERT INTO settings VALUES ($(sqlite3 /etc/x-ui/x-ui.db "SELECT IFNULL(MAX(id), 0) FROM settings"), 'webCertFile', '/etc/ssl/certs/public.key');"
x-ui restart
